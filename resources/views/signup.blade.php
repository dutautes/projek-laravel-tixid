...
@section('content')
    {{-- d-block mx-auto : membuat konten di tengah --}}
    <div class="w-75 d-block mx-auto my-5">
        {{-- isi action : dengan menambahkan http method post --}}
        <form method="POST" action="{{ route('sign_up.add') }}">
            {{-- token : kunci yang diminta server biar formulir bisa mengirim data ke server/ke controller --}}
            @csrf
            <!-- 2 column grid layout with text inputs for the first and last names -->
            <div class="row mb-4">
                <div class="col">
                    <div data-mdb-input-init class="form-outline">
                        {{-- setiap input harus punya name : identitas data agar bisa diambil di controller nya --}}
                        <input type="text" id="form3Example1" class="form-control @error('first_name') is-invalid @enderror"
                            name="first_name" value="{{ old('first_name') }}" />
                        <label class="form-label" for="form3Example1">First name</label>
                    </div>
                    {{-- @error : mengambil error validasi sesuai name yg di panggil --}}
